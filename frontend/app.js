...
function playAlertSound() {
  ...
  } catch (err) { /* браузер міг заблокувати звук без жесту користувача - не критично */ }
}
...
async function checkPendingOnLogin() {
  try {
    ...
    if (ME.role === "admin" || ME.role === "head_admin") {
      const [repairDel, parts, returns] = await Promise.all([
        api("/repair-delete-requests/pending"),
        api("/part-requests/pending"),
        api("/part-returns/pending"),
      ]);
      count += repairDel.length + parts.length + returns.length;
      if (ME.role === "head_admin") {
        const logins = await api("/login-requests/pending");
        count += logins.length;
      }
    } else if (ME.role === "parts_admin") {
      count = (await api("/part-requests/pending")).length;
    } else if (ME.role === "returns_admin") {
      count = (await api("/part-returns/pending")).length;
    }
    if (count > 0) {
      playAlertSound();
      toast(`🔔 У вас ${count} запит(ів), що очікують уваги`, "warn");
    }
  } catch (err) { /* тихо ігноруємо - це лише інформаційна перевірка */ }
}
...
function tabsForRole() {
  ...
  if (ME.role === "returns_admin") return [{ id: "returns", label: "↩️ Повернення" }];
...
function goTab(tabId) {
  ...
  const renderers = {
    ...
    requests: renderRequestsView, parts: renderPartsView, returns: renderReturnsView,
  };
...
function showMainApp() {
  ...
  const defaultTab = ME.role === "seller" ? "repairs" : (ME.role === "parts_admin" ? "requests" : (ME.role === "returns_admin" ? "returns" : "sellers"));
  goTab(defaultTab);
}

function roleLabel(role) {
  return { seller: "продавець", admin: "адмін", head_admin: "головний адмін", parts_admin: "адмін запчастин", returns_admin: "адмін повернень" }[role] || role;
}
...
function renderRepairsSellerView() {
  ...
  view.innerHTML = `
    ...
    <button class="btn secondary" id="repairHistoryBtn">📜 Історія / підрахунок</button>
  ...
  document.getElementById("repairHistoryBtn").addEventListener("click", renderRepairHistorySellerView);
}

function renderRepairHistorySellerView() {
  const area = document.getElementById("repairArea");
  const today = new Date().toISOString().slice(0, 10);
  const firstOfMonth = today.slice(0, 8) + "01";
  area.innerHTML = `
    <div class="card">
      <h3>📜 Усі квитанції за період</h3>
      <p style="color:#93a3b8;font-size:13px;">Показує прийняті й видані квитанції на вашій точці за обраний період - зручно для підрахунку.</p>
      <div class="grid2">
        <input id="historyStart" type="date" value="${firstOfMonth}">
        <input id="historyEnd" type="date" value="${today}">
      </div>
      <button class="btn" id="historyShowBtn">Показати</button>
    </div>
    <div id="repairHistoryResult"></div>`;
  document.getElementById("historyShowBtn").addEventListener("click", async () => {
    const start = document.getElementById("historyStart").value;
    const end = document.getElementById("historyEnd").value;
    const resultEl = document.getElementById("repairHistoryResult");
    resultEl.innerHTML = "<p>Завантаження...</p>";
    try {
      const rows = await api(`/repairs/report?start=${start}&end=${end}`);
      if (!rows.length) { resultEl.innerHTML = "<p>За цей період нічого не знайдено.</p>"; return; }
      const issued = rows.filter(r => r.completion_date);
      const totalCost = issued.reduce((s, r) => s + (r.cost || 0), 0);
      resultEl.innerHTML = `
        <div class="card">
          <p><b>Прийнято за період: ${rows.length}</b></p>
          <p><b>Видано за період: ${issued.length}</b></p>
          <p><b>Загальна сума виданих: ${totalCost.toFixed(2)} грн</b></p>
        </div>` +
        rows.map(r => {
          const status = r.completion_date
            ? `<span class="badge done">видано ${r.completion_date}${r.cost != null ? " | " + r.cost + " грн" : ""}</span>`
  ...
  view.innerHTML = `
    ...
    <div class="card">
      ...
      <select id="newAdminRole">
        ...
        <option value="returns_admin">Адмін повернень (лише повернення)</option>
      </select>
...
async function renderPartsView() {
  ...
  view.innerHTML = `<h2>🔗 Замовлення запчастин</h2>
    ...
    <div class="card" style="margin-top:16px;">
      <h3>↩️ Повернути запчастину</h3>
      <p style="color:#93a3b8;font-size:13px;">Якщо деталь не знадобилась чи виявився брак - сфотографуйте її і напишіть коментар (наприклад: "не знадобилась в ремонті" або "брак").</p>
      ${cameraInputHtml("returnPhotoInput")}
      <div id="returnPhotoPreview"></div>
      <label>Коментар</label><input id="returnNote" placeholder="Наприклад: брак, не працює">
      <button class="btn" id="returnSendBtn">Надіслати адміну</button>
    </div>
    <h3 style="margin-top:16px;">📜 Мої повернення</h3>
    <div id="myReturnsList">Завантаження...</div>`;

  let selectedReturnPhoto = null;
  document.getElementById("returnPhotoInput").addEventListener("change", async (e) => {
    const file = e.target.files[0];
    if (!file) return;
    selectedReturnPhoto = await resizePhotoBeforeUpload(file);
    document.getElementById("returnPhotoPreview").innerHTML = `<p style="color:#93a3b8;font-size:13px;">✅ Фото додано</p>`;
  });
  document.getElementById("returnSendBtn").addEventListener("click", async () => {
    if (!selectedReturnPhoto) { toast("Спершу сфотографуйте запчастину", "error"); return; }
    const form = new FormData();
    form.append("photo", selectedReturnPhoto);
    form.append("note", document.getElementById("returnNote").value.trim());
    try {
      await api("/part-returns", { method: "POST", form });
      toast("📨 Повернення надіслано адміну");
      renderPartsView();
    } catch (err) { toast(err.message, "error"); }
  });
  ...
  try {
    const myReturns = await api("/part-returns/mine");
    document.getElementById("myReturnsList").innerHTML = myReturns.length ? myReturns.map(r => {
      const status = r.status === "done" ? `<span class="badge done">оброблено</span>` : `<span class="badge pending">очікує</span>`;
      const img = r.photo_url ? `<img src="${authedPhotoUrl(r.photo_url)}" style="width:56px;height:56px;object-fit:cover;border-radius:8px;margin-right:10px;">` : "";
      const deleteBtn = r.status !== "done" ? `<button class="btn small danger" onclick="deleteMyPartReturn(${r.id})">🗑️</button>` : "";
      return `<div class="item-row">${img}<div class="info"><div class="name">${r.note || "(без коментаря)"} ${status}</div></div>
        ${deleteBtn}</div>`;
    }).join("") : "<p style='color:#93a3b8;'>Ще нічого не повертали.</p>";
  } catch (err) { toast(err.message, "error"); }
}

window.deleteMyPartReturn = (id) => {
  showConfirmModal("Видалити цей запис про повернення?", async () => {
    try {
      await api(`/part-returns/${id}`, { method: "DELETE" });
      toast("🗑️ Видалено");
      renderPartsView();
    } catch (err) { toast(err.message, "error"); }
  });
...
async function renderReturnsView() {
  const view = document.getElementById("view");
  const canSeeFullHistory = ME.role === "admin" || ME.role === "head_admin";
  const canEditDelete = ME.role === "admin" || ME.role === "head_admin";
  view.innerHTML = `<h2>↩️ Повернення запчастин</h2><div id="returnsList">Завантаження...</div>`;
  try {
    const rows = canSeeFullHistory ? await api("/part-returns/all") : await api("/part-returns/pending");
    const listEl = document.getElementById("returnsList");
    if (!rows.length) { listEl.innerHTML = "<p>Немає повернень.</p>"; return; }
    listEl.innerHTML = rows.map(r => {
      const status = canSeeFullHistory
        ? (r.status === "done" ? `<span class="badge done">оброблено</span>` : `<span class="badge pending">очікує</span>`)
        : "";
      const img = r.photo_url ? `<img src="${authedPhotoUrl(r.photo_url)}" style="width:72px;height:72px;object-fit:cover;border-radius:8px;margin-right:10px;">` : "";
      let actions = "";
      if (r.status !== "done") {
        actions += `<button class="btn" style="margin-top:8px;" onclick="markPartReturnDone(${r.id})">✅ Оброблено</button>`;
      }
      if (canEditDelete) {
        actions += `<button class="btn small danger" style="margin-top:8px;" onclick="deletePartReturnAdmin(${r.id})">🗑️ Видалити</button>`;
      }
      return `<div class="card"><div class="item-row">${img}<div class="info"><div class="name">${r.location} ${status}</div><div class="meta">${r.note || "(без коментаря)"} | ${r.requested_at}</div></div></div>${actions}</div>`;
    }).join("");
  } catch (err) { toast(err.message, "error"); }
}

window.markPartReturnDone = async (id) => {
  try {
    await api(`/part-returns/${id}/done`, { method: "POST" });
    toast("✅ Позначено як оброблено");
    renderReturnsView();
  } catch (err) { toast(err.message, "error"); }
};

window.deletePartReturnAdmin = (id) => {
  showConfirmModal("Видалити цей запис про повернення?", async () => {
    try {
      await api(`/part-returns/${id}`, { method: "DELETE" });
      toast("🗑️ Видалено");
