...
def returns_or_admin_required(f):
    """Доступно звичайним адмінам, головному адміну, І обмеженому
    'адміну повернень' (returns_admin) - лише перегляд і підтвердження
    повернень запчастин."""
    @functools.wraps(f)
    def wrapper(*args, **kwargs):
        if g.user["role"] not in ("admin", "head_admin", "returns_admin"):
            return jsonify({"error": "Доступно лише адміну"}), 403
        return f(*args, **kwargs)
    return wrapper
...
def push_public_key():
    public_key = vapid.get_public_key_b64()
    if not vapid.is_valid_public_key(public_key):
        return jsonify({
            "error": "Ключ VAPID_PUBLIC_KEY у налаштуваннях сервера має неправильний формат. "
                     "Найчастіша причина - у поле Value на Render випадково вставлено весь рядок "
                     "\"VAPID_PUBLIC_KEY=...\" замість самого значення після знаку =."
        }), 500
    return jsonify({"publicKey": public_key, "usingEnvKeys": vapid.using_env_keys()})
...
def list_admins_route():
    ...
    returns_admins = db.list_users(role="returns_admin")
    return jsonify(
        [{"id": r[0], "username": r[1], "role": "admin"} for r in admins] +
        [{"id": r[0], "username": r[1], "role": "parts_admin"} for r in parts_admins] +
        [{"id": r[0], "username": r[1], "role": "returns_admin"} for r in returns_admins]
    )
...
def add_admin_route():
    ...
    if role not in ("admin", "parts_admin", "returns_admin"):
...
def pending_repairs_route():
    ...
    for r in rows:
        ...
        result.append({
            ...
            "repairman_id": r[4], "repairman_name": r[5],
            "parts": [{"link": p[0], "note": p[1], "status": p[2]} for p in parts],
        })
    return jsonify(result)


@app.route("/api/repairs/colleagues", methods=["GET"])
@login_required
def repair_colleagues_route():
    """Продавці на тій самій точці - для вибору "кому передати ремонт" чи
    "хто робив ремонт" при видачі."""
    location = request.args.get("location") or seller_location(g.user)
    if not location:
        return jsonify({"error": "Не вказано торгову точку"}), 400
    rows = db.get_sellers_by_location(location)
    return jsonify([{"id": r[0], "username": r[1]} for r in rows])


@app.route("/api/repairs/<int:repair_id>/reassign", methods=["POST"])
@login_required
def reassign_repair_route(repair_id):
    """Передати ремонт іншому майстру на тій самій точці."""
    location = request.args.get("location") or seller_location(g.user)
    data = request.get_json(force=True)
    new_repairman_id = data.get("repairman_id")
    if not new_repairman_id:
        return jsonify({"error": "Оберіть майстра"}), 400
    ok = db.reassign_repair(repair_id, location, new_repairman_id)
    return jsonify({"ok": ok})
...
def complete_repair_route(repair_id):
    ...
    db.mark_repair_completed(repair_id, data["completion_date"], cost, payment_method, repairman_id)
    return jsonify({"ok": True})
...
def delete_part_request_route(request_id):
    """Видалення: адмін/головний адмін - будь-яке; продавець - лише СВОЄ і
    ЛИШЕ ПОКИ ЩЕ ОЧІКУЄ (не оброблене адміном); адмін запчастин (parts_admin)
    видаляти не може взагалі."""
    row = db.get_part_request_by_id(request_id)
    if row is None:
        return jsonify({"error": "Запит не знайдено"}), 404
    _, location, link, note, requested_by, status, repair_id = row
    ...
    if g.user["role"] == "seller" and requested_by == g.user["user_id"]:
        if status != "pending":
            return jsonify({"error": "Цю запчастину вже підтвердив адмін - видалити можна лише те, що ще очікує"}), 403
        db.delete_part_request(request_id)
        return jsonify({"ok": True})
...
def repairs_report_route():
    ...
    for r in rows:
        ...
        result.append({
            ...
            "repairman_id": r[7], "repairman_name": r[8],
            "parts": [{"link": p[0], "note": p[1], "status": p[2]} for p in parts],
        })
    return jsonify(result)
...
# ---------------------------------------------------------------------------
# Повернення запчастин (продавець -> адмін, відповідальний за повернення)
# ---------------------------------------------------------------------------
@app.route("/api/part-returns", methods=["POST"])
@login_required
def create_part_return_route():
    location = seller_location(g.user)
    if not location:
        return jsonify({"error": "Ця дія доступна лише продавцю"}), 403
    photo = request.files.get("photo")
    if photo is None:
        return jsonify({"error": "Потрібне фото запчастини"}), 400
    note = (request.form.get("note") or "").strip()
    filename = save_uploaded_photo(photo)
    return_id = db.create_part_return(location, filename, note, g.user["user_id"])
    notify_admins_push(
        "↩️ Повернення запчастини",
        f"{g.user['username']} ({location}): {note or 'без коментаря'}",
    )
    return jsonify({"ok": True, "return_id": return_id})


@app.route("/api/part-returns/pending", methods=["GET"])
@login_required
@returns_or_admin_required
def pending_part_returns_route():
    location = request.args.get("location")
    rows = db.get_pending_part_returns(location)
    return jsonify([
        {"id": r[0], "location": r[1], "photo_url": photo_url(r[2]), "note": r[3], "requested_at": r[4]}
        for r in rows
    ])


@app.route("/api/part-returns/<int:return_id>/done", methods=["POST"])
@login_required
@returns_or_admin_required
def mark_part_return_done_route(return_id):
    ok = db.mark_part_return_done(return_id, g.user["user_id"])
    return jsonify({"ok": ok})


@app.route("/api/part-returns/all", methods=["GET"])
@login_required
@admin_required
def all_part_returns_route():
    location = request.args.get("location")
    rows = db.get_all_part_returns(location)
    return jsonify([
        {"id": r[0], "location": r[1], "photo_url": photo_url(r[2]), "note": r[3],
         "requested_by": r[4], "status": r[5], "requested_at": r[6]}
        for r in rows
    ])


@app.route("/api/part-returns/mine", methods=["GET"])
@login_required
def my_part_returns_route():
    rows = db.get_part_returns_by_user(g.user["user_id"])
    return jsonify([
        {"id": r[0], "location": r[1], "photo_url": photo_url(r[2]), "note": r[3],
...
@app.route("/api/part-returns/<int:return_id>", methods=["PATCH"])
@login_required
@admin_required
def update_part_return_route(return_id):
    data = request.get_json(force=True)
    note = (data.get("note") or "").strip()
    ok = db.update_part_return(return_id, note)
    return jsonify({"ok": ok})


@app.route("/api/part-returns/<int:return_id>", methods=["DELETE"])
@login_required
def delete_part_return_route(return_id):
    """Видалення: адмін/головний адмін - будь-яке; продавець - лише СВОЄ і
    ЛИШЕ ПОКИ ЩЕ ОЧІКУЄ; адмін повернень (returns_admin) видаляти не може взагалі."""
    row = db.get_part_return_by_id(return_id)
    if row is None:
        return jsonify({"error": "Запис не знайдено"}), 404
    _, location, photo_path, note, requested_by, status = row

    if g.user["role"] in ("admin", "head_admin"):
        db.delete_part_return(return_id)
        return jsonify({"ok": True})

    if g.user["role"] == "seller" and requested_by == g.user["user_id"]:
        if status != "pending":
            return jsonify({"error": "Це повернення вже обробив адмін - видалити можна лише те, що ще очікує"}), 403
        db.delete_part_return(return_id)
        return jsonify({"ok": True})

    return jsonify({"error": "Немає прав на видалення цього запису"}), 403
