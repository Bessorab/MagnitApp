...
def _clean_env_value(raw: str, var_name: str) -> str:
    """Захист від поширеної помилки при копіюванні: якщо хтось випадково
    вставив увесь рядок "VAPID_PUBLIC_KEY=abc..." у поле Value замість
    самого значення "abc...", приберемо цей префікс і зайві пробіли/переноси."""
    value = raw.strip()
    prefix = f"{var_name}="
...
def get_private_key_pem() -> str:
    ...
    if env_scalar:
        env_scalar = _clean_env_value(env_scalar, "VAPID_PRIVATE_KEY")
...
def get_public_key_b64() -> str:
    ...
    if env_public:
        return _clean_env_value(env_public, "VAPID_PUBLIC_KEY")
...
def is_valid_public_key(public_b64: str) -> bool:
    """Перевіряє, що публічний ключ - валідний base64url і саме 65 байт
    (0x04 + 32 + 32), як і очікує браузер. Захищає від того, щоб зіпсований
    ключ (наприклад, через помилку копіювання в Environment) дійшов до
    браузера і провалився там незрозумілою помилкою atob()."""
    try:
        raw = _b64url_decode(public_b64)
    except Exception:
        return False
    return len(raw) == 65 and raw[0:1] == b"\x04"
