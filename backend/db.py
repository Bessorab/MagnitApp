    ...
    with closing(get_connection()) as conn:
        conn.execute(
            ...
            CREATE TABLE IF NOT EXISTS users (
                ...
                role TEXT NOT NULL CHECK(role IN ('head_admin', 'admin', 'parts_admin', 'returns_admin', 'seller')),
                location TEXT,
                created_at TEXT DEFAULT CURRENT_TIMESTAMP
            )
            """
        )
        # Міграція для баз, створених до появи ролей 'parts_admin'/'returns_admin':
        # стара схема забороняла ці значення через CHECK - перевіряємо тестовою
        # вставкою і, якщо стара схема, перестворюємо таблицю зі збереженням даних.
        try:
            conn.execute("INSERT INTO users (username, password_hash, role) VALUES ('__migration_test__', 'x', 'returns_admin')")
...
def get_admin_push_subscriptions():
    """Підписки всіх адмінів (head_admin, admin, parts_admin, returns_admin) -
    їм надсилаємо сповіщення."""
    with closing(get_connection()) as conn:
        cur = conn.execute(
            "SELECT ps.id, ps.endpoint, ps.p256dh, ps.auth FROM push_subscriptions ps "
            "JOIN users u ON u.id = ps.user_id WHERE u.role IN ('admin', 'head_admin', 'parts_admin', 'returns_admin')"
        )
        return cur.fetchall()
...
# ---------------------------------------------------------------------------
def create_part_return(location, photo_path, note, requested_by):
    with closing(get_connection()) as conn:
        cur = conn.execute(
            "INSERT INTO part_returns (location, photo_path, note, requested_by, requested_at) VALUES (?, ?, ?, ?, ?)",
            (location, photo_path, note, requested_by, now_str()),
        )
        conn.commit()
        return cur.lastrowid


def get_pending_part_returns(location=None):
    with closing(get_connection()) as conn:
        if location:
            cur = conn.execute(
                "SELECT id, location, photo_path, note, requested_at FROM part_returns "
                "WHERE status = 'pending' AND location = ? ORDER BY requested_at",
                (location,),
            )
        else:
...
def get_all_part_returns(location=None):
    with closing(get_connection()) as conn:
        if location:
            cur = conn.execute(
                "SELECT id, location, photo_path, note, requested_by, status, requested_at FROM part_returns "
                "WHERE location = ? ORDER BY requested_at DESC",
                (location,),
            )
        else:
            cur = conn.execute(
...
def get_part_returns_by_user(user_id):
    with closing(get_connection()) as conn:
        cur = conn.execute(
            "SELECT id, location, photo_path, note, status, requested_at FROM part_returns "
            "WHERE requested_by = ? ORDER BY requested_at DESC",
            (user_id,),
        )
        return cur.fetchall()


def get_part_return_by_id(return_id):
    with closing(get_connection()) as conn:
        cur = conn.execute(
            "SELECT id, location, photo_path, note, requested_by, status FROM part_returns WHERE id = ?",
            (return_id,),
        )
        return cur.fetchone()


def mark_part_return_done(return_id, decided_by):
    with closing(get_connection()) as conn:
        cur = conn.execute(
            "UPDATE part_returns SET status = 'done', decided_at = ?, decided_by = ? WHERE id = ? AND status = 'pending'",
            (now_str(), decided_by, return_id),
        )
        conn.commit()
        return cur.rowcount > 0


def update_part_return(return_id, note):
    with closing(get_connection()) as conn:
        cur = conn.execute("UPDATE part_returns SET note = ? WHERE id = ?", (note, return_id))
        conn.commit()
        return cur.rowcount > 0


def delete_part_return(return_id):
    with closing(get_connection()) as conn:
        cur = conn.execute("DELETE FROM part_returns WHERE id = ?", (return_id,))
        conn.commit()
        return cur.rowcount > 0
